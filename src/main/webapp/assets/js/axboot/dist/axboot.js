        ...
        if (inputs.length > 0) {
            for (var i = 0; i < inputs.length; i++) {
                ...
                if (nvl(inputs[i].getAttribute('code')) != '') {
                    ...
                    div.attr('data-tooltip-text', inputs[i].getAttribute('code'));
                } else if (nvl(inputs[i].getAttribute('CODE')) != '') {
                    var target = $(inputs[i]);
                    var div = target.parents('.input-group');
                    div.attr('data-tooltip-text', inputs[i].getAttribute('CODE'));
                }
                    ...
                    callback: function (event) {
                        if (event.attributeName == 'code'){
                            if (nvl(event.newValue) == ''){
                                ...
                                var target = $(this);
                                var div = target.parents('.input-group');
                                div.attr('data-tooltip-text', event.newValue);
                            }
                        }
                    }
                });

                if (nvl(inputs[i].getAttribute('formatter')) != '') {
                    var target = $(inputs[i]);
                    var mode = inputs[i].getAttribute('formatter');

                    // inputs[i].setAttribute('style', 'ime-mode:disabled');

                    if (mode == 'res') { //  주민번호
                        target[0].maxLength = 14;
                        target.val(target.val().replace(/(\d{6})(\d{7})/, '$1-$2'));
                    }
                    if (mode == 'company') { //  사업자번호
                        target[0].maxLength = 12;
                        target.val(target.val().replace(/(\d{3})(\d{2})(\d{5})/, '$1-$2-$3'));
                    }
                    if (mode == 'card') {  //카드번호

                        target.val(target.val().replace(/(\d{4})(\d{4})(\d{4})(\d{4})/, '$1-$2-$3-$4'));
                    }
                    if (mode == 'tel') {
                        target[0].maxLength = 13;
                    ...
                    if (mode == 'YYYYMMDD') {
                        target[0].maxLength = 10;
                        target.val(target.val().replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3'));
                    }
                    if (mode == 'money') {
                        // inputs[i].setAttribute('style', "ime-mode:disabled; text-align: right;");

                        var num = target.val().toString();
                ...
                $(inputs[i]).keydown(function (e) {
                        if (document.activeElement.tagName == 'INPUT') {
                            if (nvl($(document.activeElement)[0].getAttribute('formatter')) != '') {

                                var target = $(document.activeElement);

                                var mode = target[0].getAttribute('formatter');
                                ...
                                if (key == 0
                                    // || key == 8
                                    || key == 46 || key == 9 || (key == ctrlKey && key == vKey)
                                    || (key == ctrlKey && key == cKey)) {
                    ...
                    if (document.activeElement.tagName == 'INPUT') {
                        if (nvl($(document.activeElement)[0].getAttribute('formatter')) != '') {
                            ...
                            if (mode == 'money') {
                                var val = String(target.val().replace(/[^0-9]|-/g, ""));
                                if (val.length < 1) {
                                    return false;
                                } else if (val.length > 1) {
                                    // 데이터가 01234 이런식으로 앞에 0이 들어간채로 숫자를 입력할때 0을 제거시키기 위함
                                    if (val.indexOf("0") == 0) {
                                        val = val.substring(val.indexOf("0") + 1, val.length)
                                    }
                                }
                                // 데이터가 0000 이런식으로 숫자를 입력할때 0을 하나이외를 제거시키기 위함
                                if (Number(val) == 0) {
                                    target.val(0);
                                    return false;
                                }
...
axboot.preparePlugin = function () {
    ...
    var define = function define() {
        ...
        window.axAJAXMask = new ax5.ui.mask({
            content: '<i class="cqc-spinner9 cqc-40x cqc-zoom-in-out" style="color: #ccc;"></i>'
        });
        /**
         * 프로그래스바 형태의 마스크

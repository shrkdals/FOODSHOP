            ...
            var ACTIONS = axboot.actionExtend(fnObj, {
                ...
                PAGE_SEARCH: function (caller, act, data) {
                    axboot.ajax({
                        ...
                        url: ["event", "search"],
                        data: JSON.stringify({
                            EVENT_ST_DTE: $("#EVENT_ST_DTE").val().replace(/-/g, ""),
                            EVENT_ED_DTE: $("#EVENT_ED_DTE").val().replace(/-/g, ""),
                ...
                PAGE_SAVE: function (caller, act, data) {
                    ...
                    for (var i = 0 ; i < saveData.length ; i ++){
                        if (nvl(saveData[i].EVENT_NM) == ''){
                            qray.alert('이벤트명을 입력해주십시오.');
                            return ;
                        }
                        if (nvl(saveData[i].EVENT_ST_DTE) == ''){
                            qray.alert('이벤트 시작일을 입력해주십시오.');
                            return ;
                        }
                        if (nvl(saveData[i].EVENT_ED_DTE) == ''){
                            qray.alert('이벤트 종료일을 력해주십시오.');
                            return ;
                        }
                        if (saveData[i].EVENT_ST_DTE > saveData[i].EVENT_ED_DTE){
                            qray.alert('이벤트 시작일자가 종료일자보다 클 수 없습니다.');
                            return ;
                        }
                    }
                ...
                ITEM_ADD1: function (caller, act, data) {
                    ...
                    caller.gridView01.target.setValue(lastIdx - 1, 'EVENT_CD', GET_NO('MA','05'));
                    caller.gridView01.target.setValue(lastIdx - 1, 'EVENT_ST_DTE', dtNow);
                ...
                initView: function () {
                    ...
                    this.target = axboot.gridBuilder({
                        ...
                        columns: [
                            ...
                            {key: "EVENT_CD", label: "이벤트코드", width: 150 , align: "center" , editor: false, sortable: true,},
                            {key: "EVENT_NM", label: "이벤트명", width: 150 , editor: {type: "text"}, align: "left",sortable: true,},
                            {key: "EVENT_ST_DTE", label: "이벤트시작일자", width: 150 , align: "center" ,sortable: true,
                                editor: {type: "date", config: {separator: "-"}},
                                formatter: function () {
                                    var returnValue = this.item.EVENT_ST_DTE;
                                    if (nvl(this.item.EVENT_ST_DTE, '') != '') {
                                        this.item.EVENT_ST_DTE = this.item.EVENT_ST_DTE.replace(/\-/g, '');
                                        returnValue = $.changeDataFormat(this.item.EVENT_ST_DTE, 'YYYYMMDD');
                                    }
                                    return returnValue;
                                }
                            },
                            {key: "EVENT_ED_DTE", label: "이벤트종료일자", width: 150 , align: "center", sortable: true,
                                editor: {type: "date", config: {separator: "-"}},
                                formatter: function () {
                                    var returnValue = this.item.EVENT_ED_DTE;
                                    if (nvl(this.item.EVENT_ED_DTE, '') != '') {
                                        this.item.EVENT_ED_DTE = this.item.EVENT_ED_DTE.replace(/\-/g, '');
                                        returnValue = $.changeDataFormat(this.item.EVENT_ED_DTE, 'YYYYMMDD');
                                    }
                                    return returnValue;
                                }
                            },
        ...
        <div role="page-header" id="pageheader">
            <ax:form name="searchView0">
                <ax:tbl clazz="ax-search-tb1" minWidth="500px">
                    <ax:tr>
                        <ax:td label='이벤트 시작일자' width="350px">
                            <div class="input-group" data-ax5picker="basic">
                                <input type="text" class="form-control" placeholder="yyyy/mm/dd" id="EVENT_ST_DTE" formatter="YYYYMMDD">
                                <span class="input-group-addon">~</span>
                                <input type="text" class="form-control" placeholder="yyyy/mm/dd" id="EVENT_ED_DTE" formatter="YYYYMMDD">
                                <span class="input-group-addon"><i class="cqc-calendar"></i></span>
                            </div>
                        </ax:td>
                        <ax:td label='이벤트명' width="350px">
                            <div class="input-group" style="width:100%">
                                <input type="text" class="form-control" name="EVENT_NM" id="EVENT_NM"
                                       style="width:100%"/>
                            </div>
                        </ax:td>
                    </ax:tr>

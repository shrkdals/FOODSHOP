        ...
        <script type="text/javascript">
            ...
            var ACTIONS = axboot.actionExtend(fnObj, {
                ...
                PARTNER_CREATE: function (caller, act, data) {
                    menuInfo = {progPh:"/jsp/ensys/ma/p_cz_q_ma_partner_m.jsp", menuId:"10401", parentid:"4", id:"10401", progNm:"거래처정보관리", menuNm:"거래처정보관리", progPh:"/jsp/ensys/ma/p_cz_q_ma_partner_m.jsp", name:"거래처관리"};
                    sessionStorage.setItem("prevmenuid", "10502");
                    try{
                        parent.fnObj.tabView.close("10401")
                    }catch(e){
                ...
                DOCU_CREATE: function (caller, act, data) {
                    ...
                    var result = $.DATA_SEARCH("apTaxBill","getSelectCheckImsi",{ CD_COMPANY : SCRIPT_SESSION.cdCompany, NO_TPDOCU : tax.join('|')});
                    if(result.map.YN == 'Y'){
                        qray.alert("이미 저장된 데이터가 존재합니다.");
                        return false;
                    }
                    userCallBack = function (e) {
                        modal.close({
                            callback : function(){
                                console.log(e);
                                if(e.YN == 'Y'){
                                    menuInfo = {menuId: "14104", id: "14104",  menuNm: "전표등록 NEW" ,name: "업체전용(개발)" , parentId : "14" , progNm: "전표등록관리", progPh: "/jsp/ensys/gl/p_cz_q_gl_docu_m2.jsp"};
                                    sessionStorage.setItem("prevmenuid", "10502");
                                    parent.fnObj.tabView.urlSetData({RESULT : createList.result , DT_ACCT : e.DT_ACCT , REMARK : e.REMARK , CD_DOCU : e.DOCU_TP });
                                    try{
                                        parent.fnObj.tabView.close("14104")
                                    }catch(e){
                        ...
                        columns: [
                            {
                                ...
                                formatter: function () {
                                    ...
                                    if (nvl(this.item.NO_DOCU) != '' || this.item.PUSAIV_IMSI_YN != '미저장') {
                                        if (nvl(this.item.CD_PARTNER) == ''){
                                            if (nvl(CHK, 'N') == 'N') {
                            ...
                            {key: "PUSAIV_IMSI_YN", label: "전표발행유무", width: 90, align: "left", editor: false, sortable: true,},
                            //{key: "PUSAIV_YN", label: "전표발행유무", width: 90, align: "left", editor: false},
                            ...
                            {
                                key: "DT_INEND", label: "지급요청일자", width: 100, align: "center", editor: false, hidden:true, sortable: true,
                                formatter: function () {
            ...
            function isChecked(data) {
                ...
                var chkBox = $("div .columnBox");

                for (var i = 0; i < chkBox.length; i++) {
                    if (chkBox[i].getAttribute('data-ax5grid-checked') == "true") {
                        var index = chkBox[i].id.split("columnBox")[1];
            ...
            var detailCnt = 0;
            $(document).on('click', '.columnBox', function (caller) {
                console.log("columnBox : ", this);
                var index = this.id.split('columnBox')[1];

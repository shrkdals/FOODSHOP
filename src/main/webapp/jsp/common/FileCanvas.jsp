                 ...
                 axboot.call({
                     ...
                     axboot.ajax({
                         ...
                         callback: function (e) {
                             ...
                             if (((initData.CALL_BACK.FILE_EXT === 'jpg') || (initData.CALL_BACK.FILE_EXT === 'gif') || (initData.CALL_BACK.FILE_EXT === 'png'))){
                                 ...
                                 axboot.call({
                                     ...
                                     axboot.ajax({
                                         ...
                                         callback: function (e) {
                                             ...
                                             if (((data.FILE_EXT === 'jpg') || (data.FILE_EXT === 'gif') || (data.FILE_EXT === 'png'))){
	                                             $(".preview").append('<img src="' + ORGN_FILE_PATH + '" class="img-responsive"/>');
	                                             $(".img-preview").append('<img src="' + FILE_PATH + '" id="canvas_crop"/>');
                                             }
                                             changesize();
...
<ax:layout name="base">
	...
	<script type="text/javascript" src='/assets/js/crop/cropper.js'></script>
     <script>
         ...
         var ACTIONS = axboot.actionExtend(fnObj, {
                 ...
                 axboot.call({
                     ...
                     axboot.ajax({
                         ...
                         callback: function (e) {
                             ...
                             $("#FILE_BYTE").val(initData.CALL_BACK.FILE_BYTE);
                             $("#FILE_SIZE").val(initData.CALL_BACK.FILE_SIZE);
                             $("#FILE_NAME").val(initData.CALL_BACK.FILE_NAME);
                             $("#ORGN_FILE_PATH").val(ORGN_FILE_PATH);
                             $("#FILE_PATH").val(FILE_PATH);
                             $("#REAL_ORGN_FILE_PATH").val(initData.CALL_BACK.FILE_PATH + "/original");
                             $("#REAL_FILE_PATH").val(initData.CALL_BACK.FILE_PATH);
                                 ...
                                 axboot.call({
                                     ...
                                     axboot.ajax({
                                         ...
                                         callback: function (e) {
                                             ...
                                             $("#FILE_BYTE").val(data.FILE_BYTE);
                                             $("#FILE_SIZE").val(data.FILE_SIZE);
                                             $("#FILE_NAME").val(data.FILE_NM);
                                             $("#ORGN_FILE_PATH").val(ORGN_FILE_PATH);
                                             $("#FILE_PATH").val(FILE_PATH);
                                             $("#REAL_ORGN_FILE_PATH").val(data.FILE_PATH + "/original");
                                             $("#REAL_FILE_PATH").val(data.FILE_PATH);
                                             $(".preview").append('<img src="' + ORGN_FILE_PATH + '" class="img-responsive"/>');
                                             $(".img-preview").append('<img src="' + FILE_PATH + '" id="canvas_crop"/>');
                                             changesize();
                                         }
             ...
             PAGE_SAVE: function () {
            	 if ($("#fileCropper").css('display') == 'block'){
					qray.alert('자르기 편집 중입니다.<br>편집을 마무리 후 저장해주십시오.');
					return false;
            	 }
                 if (nvl(cropFile) == '' || nvl(orgnFile) == ''){
                     qray.alert('변경된 데이터가 없습니다.');
             ...
             IMG_DOWNLOAD: function(){
                 if (nvl($("#ORGN_FILE_PATH").val()) == '' || nvl($("#FILE_PATH").val()) == '' ){
					qray.alert('파일을 저장한 후 다운로드를 진행해주십시오.');
					return false;
				}
				...
				if (window.navigator.msSaveBlob) { // IE
                    ...
                    xhr.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            window.navigator.msSaveOrOpenBlob(_blob, $("#ORGN_FILE_NAME").val());
                        }
                    };
                    xhr.send(JSON.stringify({
                    	FILE_PATH : $("#REAL_ORGN_FILE_PATH").val(),
                    	FILE_NAME : $("#FILE_NAME").val(),
                    	FILE_EXT  : $("#FILE_EXT").val(), 
                    }));
				}else{
					var link = document.createElement('a');
					link.setAttribute("href", $("#ORGN_FILE_PATH").val());
                    link.setAttribute("download", $("#ORGN_FILE_NAME").val());
                    link.click();
				}
             },
             FILE_LOAD: function () {
                                    	 ...
                                    	 if ($(".img-responsive").length > 0) {
                                             qray.confirm({
                                                 ...
                                                 if (this.key == "ok") {
                                                     ...
                                                     $("#FILE_BYTE").val(returnValue.FILE_BYTE);
                                                     $("#FILE_SIZE").val(returnValue.FILE_SIZE);
                                                     $("#FILE_NAME").val(returnValue.FILE_NAME);
                                                     $("#ORGN_FILE_PATH").val('');
                                                     $("#FILE_PATH").val('');
                                                     $(".preview").append('<img src="' + returnValue.FILE_PATH + '" class="img-responsive"/>');
                                                     $(".img-preview").append('<img id="canvas_crop" src="' + returnValue.FILE_PATH + '"></img>');
         ...
         fnObj.pageButtonView = axboot.viewExtend({
             initView: function () {
                 axboot.buttonClick(this, "data-page-btn", {
                     ...
                     "download": function(){
                    	 ACTIONS.dispatch(ACTIONS.IMG_DOWNLOAD);
                     },
                     "save": function () {
                         ACTIONS.dispatch(ACTIONS.PAGE_SAVE);
                     },
         ...
         fnObj.pageStart = function () {
             ...
             if (initData.CALL_BACK != '' && initData.CALL_BACK != null && initData.CALL_BACK != undefined  ){
            	 ACTIONS.dispatch(ACTIONS.CALL_BACK);
             }else{
            	 ACTIONS.dispatch(ACTIONS.PAGE_SEARCH);
             }
    ...
    <jsp:body>
        <div data-page-buttons="">
            <div class="button-warp">
                <span style="margin-right: 25px">* 파일의 해상도를 1500 x 1500 이상으로 맞춰주십시오.</span>
                <button type="button" class="btn btn-reload"
                        data-page-btn="reload" onclick="window.location.reload();">
                    <i class="icon_reload"></i>
                </button>
                <button type="button" class="btn btn-info"
                        data-page-btn="download"><i class="cqc-download"></i>다운로드
                </button>
                <button type="button" class="btn btn-info"
                        data-page-btn="save" id="save">
                    <i class="icon_save"></i>임시저장
            ...
            <ax:split-panel width="400" style="padding-left: 10px;"
                ...
                <ax:form name="formView01">
                    <div data-fit-height-aside="form-view-01">
                        ...
                        <ax:tbl clazz="ax-form-tbl">
                        <input type="hidden" data-ax-path="FILE_NAME" id="FILE_NAME" readonly value=""/>
                        <input type="hidden" data-ax-path="ORGN_FILE_PATH" id="ORGN_FILE_PATH" readonly value=""/>
                        <input type="hidden" data-ax-path="FILE_PATH" id="FILE_PATH" readonly value=""/>
                        <input type="hidden" data-ax-path="REAL_ORGN_FILE_PATH" id="REAL_ORGN_FILE_PATH" readonly value=""/>
                        <input type="hidden" data-ax-path="REAL_FILE_PATH" id="REAL_FILE_PATH" readonly value=""/>
                            <ax:tr labelWidth="80px">
                                <ax:td label="파일명" width="100%">
                                    <input type="text"
                                           data-ax-path="ORGN_FILE_NAME" id="ORGN_FILE_NAME"

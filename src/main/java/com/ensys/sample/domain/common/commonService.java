...
public class commonService extends BaseService {
    ...
    public String getSeq(String cdCompany, String cdModule, String cdClass, String docuYm) {
        Map<String, String> param = new HashMap();
        param.put("cdCompany", cdCompany);
        param.put("cdModule", cdModule);
        param.put("cdClass", cdClass);
        param.put("docuYm", docuYm);
        commonMapper.getSeq(param);
        return param.get("no");
    }


    public List<Map<String, String>> getAuthSeq() {
        SessionUser user = SessionUtils.getCurrentUser();

        HashMap<String, Object> param = new HashMap<String, Object>();
        param.put("CD_COMPANY", user.getCdCompany());
        return commonMapper.getWebSeq(param);
    }


    public List<Map<String, String>> getCodeList(common common) {
        return commonMapper.getCodeList(common);
    }
    ...
    public Map<String, Object> selectMenu() {
        ...
        HashMap<String, Object> param = new HashMap<String, Object>();
        param.put("CD_COMPANY", user.getCdCompany());
        param.put("ID_USER", user.getIdUser());
        param.put("CD_GROUP", user.getCdGroup());

        Map<String, Object> Map = new HashMap();

        Map.put("menu", commonMapper.selectMenu(param));
        Map.put("prog", commonMapper.selectProg(param));
        return Map;
    }
    ...
    public void goFtp() {
        ...
        try {
            ...
            client.setControlEncoding("UTF-8"); // connection 환경에서 UTF-8의 인코딩 타입을 사용한다.
            client.connect("rahan2002.cafe24.com", 21);
            int resultCode = client.getReplyCode();

            if (!FTPReply.isPositiveCompletion(resultCode)) {   // 접속시 에러가 나오면 콘솔에 에러 메시지를 표시하고 프로그램을 종료한다.
                System.out.println("FTP server refused connection.!");
                return;
            } else {

                client.setSoTimeout(1000);  // 파일 전송간 접속 딜레이 설정 (1ms 단위기 때문에 1000이면 1초)

                if (!client.login("rahan2002", "rahan123!@")) { // 로그인을 한다.

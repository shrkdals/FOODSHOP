...
public class SMSComponent {
	...
	// 실제 발송하는 과정입니다.
	public String SendMsg(String[] strDestList, String strCallBack, String strSubject, String strDate, String strData,
			int nCount) throws IOException {
		...
		// 발송내역 유니코드로 변환.
		String jsonB = "\"msg\":\"" + strData.trim() + "\"," + "\"title\":\"" + strSubject.trim() + "\"}";
		jsonB = encode(jsonB);

		for (int k = 0; k < nCount; k++) {
			// 나머지 JSON 코드를 생성한다.
			String json = "{" + "\"key\":\"" + SMSConfig.TOKEN + "\"," + "\"tel\":\"" + strDestList[k] + "\","
					+ "\"cb\":\"" + strCallBack + "\"," + "\"date\":\"" + strDate + "\"," + jsonB;
	 ...
	 *
	 * @param String text 원하는 문자열입니다. int startIndex 시작시점입니다. int length 자를 길이입니다.
